...
for test_case in test_cases:
    with np.load(test_data_path + test_case, allow_pickle=True) as d:
        ...
        mask_RK = d['mask_right_kidney'].copy()  # Ground truth for Right Kidney

    # Define output masks for Right Kidney
    out_masks = {model_name: np.zeros_like(image) for model_name in model_paths.keys()}
    base_mask_RK = np.zeros_like(image)

    # Find the first slice with significant right kidney presence
    start_slice = next((slice for slice in range(image.shape[2]) if np.sum(mask_RK[:, :, slice]) > 500), 0)
    ...
    for slice in range(start_slice, start_slice + 5):
        for model_name, model_path in model_paths.items():
            ...
            # Extract segmentation output for Right Kidney
            try:
                out_masks[model_name][:, :, slice] = output["RK"]  # CHAOS model format
            except KeyError:
                try:
                    out_masks[model_name][:, :, slice] = output["right_kidney"]  # AMOS model format
                except KeyError:
                    print(f"Warning: Right Kidney not found in {model_name}")

            # Assign ground truth mask
            base_mask_RK[:, :, slice] = mask_RK[:, :, slice]

    # Compute Dice Score and IoU for each model
    for model_name, out_mask in out_masks.items():
        numerator = np.sum(out_mask * base_mask_RK)
        denominator = np.sum(out_mask) + np.sum(base_mask_RK)
        ...
        results.append([test_case, model_name, "Right Kidney", dice, iou])

# Convert results to DataFrame
df = pd.DataFrame(results, columns=["Test Dataset", "Model", "Organ", "Dice Score", "IoU"])

# Save to Excel file
output_excel_path = "/Users/dibya/MyThesisDatasets/final_model/right_kidney_comparison.xlsx"
